...
class Class01sController extends AdminController
{
    ...
    protected function detail($id)
    {
        ...
        $show->field('id', __('Ques_s'))->unescape()->as(function ($id) {
            $content_ok = [];
            //題目
            ...
            $class01Data = DB::table('class01s')->select('company','ques_s')->where('id',$id)->get();
            //答案
            ...
            if(!empty( $newclass01Data[0]['ques_s'] )){
                ...
                //要傳到PDF的因此轉json
                $content_json = json_encode($content_ok);
                // Log::info($content_ok);
                $dom = "<h3>單選</h3>";
                $dom .= "<a class='btn btn-primary' href='/getDate?val={$newclass01Data[0]['ques_s']}' target='_blank'>下載</a>";
                $dom .= "<a class='btn btn-danger' href='/pdf?val={$content_json}&val2={$newclass01Data[0]['company']}' target='_blank'>PDF下載</a>";
                $dom .= "<ul>";
        ...
        //         $dom .= "<a class='btn btn-danger' href='/pdf?val=$content_json' target='_blank'>PDF下載</a>";
        //         $dom .= "<ul>";
        //         foreach($content_ok as $item){
        //             $dom .= "<li>[ {$item['ansNO']}.{$item['ansTxt']} ] - {$item['content']}</li>";
        //         }
